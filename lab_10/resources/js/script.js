  ...
  $.ajax({url:url, dataType:"jsonp"}).then(function(data) {
    ...
    */
    ...
    function getKey(i){
        ...
        console.log ( data.forecast[Object.keys(data.forecast)[i]] ); 
        return data.forecast[Object.keys(data.forecast)[i]];
    }
    

    for ( var i = 0; i < 6; i++ ){
      var forecastElement = getKey(i); 
      var myCard = 
      ' <div style=width: 20%;">  \
          <div class="card">   \
            <div class="card-body"> \
              <h5 class="card-title">' + dayOfWeek /*List Day of the Week Here*/ +
              '<p class="card-text"> '
                + '<br></br> High:' + forecastElement.maxTemp/*List Temperature High */ 
                + '<br></br> Low:' + forecastElement.mintemp /*<List Temperature Low>*/  
                + '<br></br> Sunrise:' + forecastElement.astro.sunrise /* List Time of Sunrise */ 
                + '<br></br> Sunset:' + forecastElement.astro.sunset /*List Time of Sunset*/
              +
              '</p>\
            </div> \
          </div> \
        </div> ';
      document.getElementById("5_day_forecast").innerHTML += myCard;
      
    }  /* -------------------------------------------------------------------------------------------------------------
    ------------------------------- leaving off here ... next step = assigning to innerHTML attribute ----------------- 
    ------------------------------------------------------------------------------------------------------------------*/
